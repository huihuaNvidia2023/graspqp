...
from graspqp.optim.state import TrajectoryState, ReferenceTrajectory, ResultSelector
...
class TestReferenceTrajectory(unittest.TestCase):
    ...
    def test_basic_creation(self):
        ...
        # New API: contact_fingers (list of finger names) and n_contacts (int)
        ...
        self.assertEqual(ref.contact_fingers, ["thumb", "index", "middle"])
    ...
    def test_no_finger_constraints(self):
        """Test creation without finger constraints (uniform sampling mode)."""
        hand = torch.randn(self.B, self.T, self.D_hand)
        obj = torch.randn(self.B, self.T, self.D_obj)
        ...
        ref = ReferenceTrajectory(
            hand_states=hand,
            object_states=obj,
            contact_fingers=None,  # No constraints = uniform sampling
            n_contacts=self.n_contacts,
        )
        
        self.assertIsNone(ref.contact_fingers)
        self.assertEqual(ref.n_contacts, self.n_contacts)
...
class TestResultSelector(unittest.TestCase):
    ...
    def test_select_best_valid(self):
        ...
        best_state, best_energy, success = ResultSelector.select_best_valid(
            state, energies, valid
        )
