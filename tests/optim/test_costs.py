...
class TestReferenceTrackingCost(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.reference = ReferenceTrajectory(
            hand_states=self.ref_hand,
            object_states=self.ref_obj,
...
class TestVelocitySmoothnessCost(unittest.TestCase):
    ...
    def test_component_selection(self):
        ...
        # Hand-only
        cost_hand = VelocitySmoothnessCost(
            weight=1.0, config={"component": "hand"}
        )
        # Object-only  
        cost_obj = VelocitySmoothnessCost(
            weight=1.0, config={"component": "object"}
        )
        # Both
        cost_both = VelocitySmoothnessCost(
            weight=1.0, config={"component": "both"}
        )
